...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $sql = 'SELECT c.*, u.identificador AS unidad_identificador FROM cobros c JOIN unidades u ON u.id = c.unidad_id WHERE c.organizacion_id = ?';
    $types = 'i'; $params = [$organizacion_id];
    if ($unidad_id) { $sql .= ' AND c.unidad_id = ?'; $types .= 'i'; $params[] = $unidad_id; }
    if ($sector_id) { $sql .= ' AND c.sector_id = ?'; $types .= 'i'; $params[] = $sector_id; }
    if ($desde) { $sql .= ' AND c.fecha >= ?'; $types .= 's'; $params[] = $desde; }
    if ($hasta) { $sql .= ' AND c.fecha <= ?'; $types .= 's'; $params[] = $hasta; }
    ...
    echo json_encode(['cobros' => $items]);
    exit;
}

$input = json_decode(file_get_contents('php://input'), true) ?? [];

// POST create (registrar cobro)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $recibo_num = null;
    $res = $conn->query('SELECT LPAD(COALESCE(MAX(id)+1,1), 6, "0") AS nextnum FROM cobros WHERE organizacion_id = '.intval($organizacion_id));
    if ($row = $res->fetch_assoc()) { $recibo_num = $row['nextnum']; }

    $stmt = $conn->prepare('INSERT INTO cobros (organizacion_id, sector_id, unidad_id, fecha, monto, metodo, referencia, recibo_num, usuario_id) VALUES (?,?,?,?,?,?,?,?,?)');
    ...
    if (!$stmt->execute()) { http_response_code(500); echo json_encode(['error'=>'No se pudo registrar cobro']); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT' && isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare('UPDATE cobros SET estado = ? WHERE id = ?');
    $stmt->bind_param('si', $estado, $id);
    if (!$stmt->execute()) { http_response_code(500); echo json_encode(['error'=>'No se pudo actualizar cobro']); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE' && isset($_GET['id'])) {
    ...
    if (!$stmt->execute()) { http_response_code(500); echo json_encode(['error'=>'No se pudo eliminar cobro']); exit; }
    echo json_encode(['ok'=>true]);
    exit;
}
