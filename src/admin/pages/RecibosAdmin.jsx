...
// UI Flow:
// 1. Selector de organización (si el usuario tiene varias) -> periodo (YYYY-MM) -> listado de recibos
// 2. Acciones por recibo: Ver detalles (expandir fila), Registrar pago, Anular recibo
// 3. Modal rápido para registrar pago (monto, método, referencia, observación)
// 4. Resumen superior por estado y montos.
...
export default function RecibosAdmin(){
  ...
  // Leer query params para deep link desde resumen
  const params = new URLSearchParams(location.search);
  const qpOrg = params.get('organizacion_id') || '';
  const qpPeriodo = params.get('periodo') || '';
  const qpSector = params.get('sector_id') || '';

  const [organizacionId,setOrganizacionId] = useState(qpOrg);
  const [periodo,setPeriodo] = useState(qpPeriodo || new Date().toISOString().slice(0,7)); // YYYY-MM actual
  ...
  const [sectorId,setSectorId] = useState(qpSector);

  // Cargar organizaciones para selector
  useEffect(()=>{ (async()=>{
    if(!usuario) return;
    try {
  ...
  useEffect(()=>{ fetchRecibos(); /* eslint-disable-next-line */ },[organizacionId,periodo,sectorId]);

  // Sin organización seleccionada: si viene por deep link no tocar; si solo 1 org se auto-set arriba
  useEffect(()=>{
    // Sincronizar URL con selección actual para compartir enlace
    if(!organizacionId) return; if(!periodo) return;
    const p = new URLSearchParams(); p.set('organizacion_id',organizacionId); p.set('periodo',periodo); if(sectorId) p.set('sector_id',sectorId);
    const newUrl = `/admin/recibos/detalle?${p.toString()}`;
    if(newUrl !== location.pathname + location.search){
      navigate(newUrl, { replace:true });
    }
  },[organizacionId,periodo,sectorId]);
  ...
  return (
    <section className="px-2 sm:px-4 py-4 w-full">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Recibos Detalle</h1>
        <button className="text-sm text-blue-700 underline" onClick={()=>navigate('/admin/recibos')}>Volver al resumen</button>
      </div>
